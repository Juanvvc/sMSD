 ...
 * of messages. In order to convert or connect a connection to/into stndard
 * streeams (such as the ones in java.io), see TransformConnection classes.
...
public class Connection{
    ...
    /** Connection to get a remote cache having template */
    public static final int GET=4;
    /** Connection to get a remote cache not having template */
    public static final int GET_NOT=5;
    /** An special connection to use virtual connections an messages
     * inside it.
     */
     ...
     * Is null, manual calls to receive are needed */
    ...
    private class TriggerEvent extends Thread{
        ...
        public TriggerEvent(Connection c,Message m,int type,Vector l){
            ...
            for(int i=0; i<l.size(); i++){
                listeners.addElement(l.elementAt(i));
            }
            start();
        }
        ...
        public void run(){
            ...
            for(int i=0; i<listeners.size(); i++){
                ConnectionListener l=(ConnectionListener)listeners.elementAt(i);
                l.event(e);
            }
    ...
    /** Send a byte array to the other side of the connection.
     * @param b Thearray to send
     * @throws Exception If the array can not be sent */
    public void sendBytes(byte[] b) throws Exception{
        send(new Message(b,idfrom,idto,type));
    }

    /** Receive a message from the connection. Blocks until received.
     *
     * Keep in mind you can get a CLOSE message calling to this method
     * if there are virtual connection inside this one.
     ...
     * The type of the connection is just for discriminating connection
...
/** This class represents a loop back connectiono */
class LocalConnection extends Connection{
     ...
     * Also set the virtual parameter of the message
     */
    public void send(Message m) throws Exception{
        if(other==null)
    ...
    /** Receive a new message. This method blocks until a new message
     * is arrived.
     */
    public Message receive() throws Exception{
        if(closed){
