 ...
 *
 * There is a lot of buffers in the way to the service: an internal one
 * in the client, another in the server, a couple of them in the bridges...
 * Keep in mind we are trying to connect little devices and big ones, with
 * enormous defferences in their speeds. Try to send enought byes not to
 * overflow buffers: by default, the buffer sizes are 4KB, so do a flush()
 * every 1KB. Much better if you can develop a protocol over the streams
 * to wait for a confirmation after 1KB of information send.
...
public class TransformConnection{
     ...
     * @throws NulPointerException If con is null
     */
     ...
     * @throws NulPointerException If con or cs are null
     */
     ...
     * @throws NulPointerException If from or to are null */
...
/** Convert from two Streams to a connection.
...
//    private static final Logger logger=Logger.getLogger(MInputStream.class); //@@l
    ...
    public void event(ConnectionEvent e){
        ...
        switch(e.getType()){
        case ConnectionEvent.ERROR:
            System.err.println("An error from the connection: "+
        ...
        case ConnectionEvent.RECEIVED:
            ...
            synchronized(buffer){
                ...
                if(w>buffer.length-now){
                    System.err.println("Discarding "+(buffer.length-now-w)+"!");
                }
...
/** This class converts an OutputStream into a Connection */
class MOutputStream extends OutputStream{
    ...
    public void write(byte[] b,int off,int len) throws IOException{
        ...
        // if the size is greather than the buffer size, cut b in pieces.
